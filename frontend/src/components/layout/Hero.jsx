...
const Hero = () => (
  <div className="relative overflow-hidden bg-hero-gradient">
    ...
    <div className="relative max-w-7xl mx-auto px-4 py-14 sm:py-20 grid md:grid-cols-2 gap-8 items-center">
      <div className="animate-slide-up">
        <span className="inline-block text-brand-300 bg-white/10 text-xs font-semibold tracking-widest uppercase px-3 py-1 rounded-full mb-4">
        ...
        <p className="mt-4 text-ink-300 text-base max-w-md">
          Hardware, sanitaryware and lighting — quality products, honest prices, and expert advice from a name you know.
        </p>

        <div className="mt-7 flex flex-wrap gap-3">
          <Link to="/?category=hardware" className="bg-brand-gradient text-white font-semibold px-6 py-3 rounded-full shadow-card hover:opacity-90 transition">
            Shop Hardware
          </Link>
          <Link to="/?category=lightings" className="bg-white/10 text-white font-semibold px-6 py-3 rounded-full border border-white/20 hover:bg-white/20 transition">
            Explore Lightings
          </Link>
        </div>
      ...
      <div className="hidden md:flex justify-center">
        <div className="grid grid-cols-2 gap-4 w-full max-w-sm">
          {[
            ...
            <div
              key={tile.label}
              className="bg-white/10 backdrop-blur border border-white/15 rounded-2xl p-6 flex flex-col items-center gap-2.5 text-center"
            >
              <span className="w-11 h-11 rounded-xl bg-brand-gradient flex items-center justify-center">
                <svg viewBox="0 0 24 24" className="w-5 h-5 text-white" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  <path d={tile.icon} />
                </svg>
              </span>
              <span className="text-white text-sm font-semibold">{tile.label}</span>
            </div>
          ))}
        </div>
      </div>
